...
int main()
{
    ...
    string word = chooseWord();//chọn từ
    string guessedWord = string(word.length(), '-');//điền các chữ cái đã đoán vào dấu gạch
    string badGuesses = "";// các từ đoán sai
...
void renderGame(const string& guessedWord, const string& badGuesses)
{
    ...
    for (int i = 0; i < PATCH_LINES; i++) cout << endl;//đẩy lùi FIGURE xuống dưới
    
    long badGuessCount = badGuesses.length();
...
char readAGuess()
{
    ...
    cout << "Your guess: ";
    cin >> input;
    return tolower(input);//convert tất cả về chữ thường
}
